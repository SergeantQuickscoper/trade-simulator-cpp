...
TradeResult Simulator::simulateTrade(double orderSize,
    ...
    result.marketImpact = marketImpactModel_->calculateImpact(orderSize, limitPrice, timeHorizon);
    ...
    result.fees = feeModel_->calculateFee(orderSize, result.executedPrice, isMaker, orderType);
...
DetailedTradeMetrics Simulator::calculateTradeMetrics(double orderSize,
    ...
    metrics.expectedMarketImpact = marketImpactModel_->calculateImpact(
        orderSize,
        metrics.midPrice,
        timeHorizon
    );
    ...
    metrics.expectedFees = feeModel_->calculateFee(
        orderSize,
        metrics.midPrice,
        isMaker,
        orderType
...
double Simulator::estimateInternalLatency() {
    ...
    // Generate random jitter (simplified)
